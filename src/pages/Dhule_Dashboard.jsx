...
/** Format current date: "May 24, 2025 / Saturday" */
...
export default function Dhule_Dashboard() {
    ...
    const fetchDashboard = useCallback(async () => {
        try {
            ...
            // Bind the date from API if it exists, otherwise fallback to current date
            if (res.data?.lastUpdated) {
                setLastUpdated(res.data.lastUpdated);
            } else {
                const { date, day } = getFormattedDate();
                setLastUpdated(`${date} | ${day}`);
            }
                ...
                {!loading && !error && (
                    <div className="hn-grid">
                        {cards.map((card) => {
                            ...
                            const isClickable = !!(card.link || meta.route);
                            return (
                                <HomeCard_NEW
