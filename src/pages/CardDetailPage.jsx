...
const CARD_COLUMNS_MAP = {
  ...
  "illegal hoarding": { cols: ["Panchnama Count", "Total Notices", "Notice Gen. Amt (In Lacs)", null] },
  "illegal hording": { cols: ["Panchnama Count", "Total Notices", "Notice Gen. Amt (In Lacs)", null] },
  "mandap": { cols: ["Total Applications", "Approved", "Rejected", "Pending"] },
};
...
function BarChart({ cols, metrics, color, months, apiData, chartType }) {
  ...
  useEffect(() => {
    ...
    const option = {
      ...
      grid: { left: "3%", right: "4%", bottom: cols.length > 4 ? "25%" : "15%", top: "8%", containLabel: true },
      xAxis: chartType === 'pie' ? { show: false } : {
        type: "category",
        data: months,
        axisLine: { show: false },
        axisTick: { show: false },
...
function matchApiColumn(apiRow, colLabel) {
  ...
  const keys = Object.keys(apiRow);
   const customMap = {
    ...
    "panchnama count": "panchnama_count",
    "total notices": "notice_count",
    "notice gen. amt (in lacs)": "notice_amount"
  };
...
export default function CardDetailPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
          ...
          params: { ulbId, flag: titleKey === 'illegal hording' ? 'ILHORD' : titleKey }
  ...
  const statCards = activeCols.map(col => {
    ...
    if (titleKey === 'water tax' && col.toLowerCase() === 'total demand' && watTotalDemandCr !== null) {
      totalVal = watTotalDemandCr * 10000000; 
    }
            ...
            <div className="cd-charts-section">
              <div className="cd-chart-card">
                <div className="cd-chart-header">
                  ...
                  <div className="cd-chart-controls" style={{ display: 'flex', gap: '8px' }}>
                    <button 
                      onClick={() => setChartType('bar')}
                      className={`cd-chart-btn ${chartType === 'bar' ? 'active' : ''}`}
                    >
                      Bar chart
                    </button>
                    <button 
                      onClick={() => setChartType('area')}
                      className={`cd-chart-btn ${chartType === 'area' ? 'active' : ''}`}
                    >
                      Area chart
                    </button>
                    <button 
                      onClick={() => setChartType('pie')}
                      className={`cd-chart-btn ${chartType === 'pie' ? 'active' : ''}`}
                    >
                      Pie chart
                    </button>
                  </div>
                </div>
                <BarChart cols={tableActiveCols} metrics={metrics} color={color} months={activeMonth ? [activeMonth] : displayMonths} apiData={apiData} chartType={chartType} />
              </div>
            </div>
