...
const CARD_META_LIST = [
  ...
  {
    color: "red",
    route: "https://crmdashboard.onlinevvcmc.in",
    icon: (
...
function getCardMeta(title, index) {
  // First, try title-based lookup to avoid API order mismatch
  if (title) {
...
export default function Home_NEW() {
  ...
  const handleCardClick = (card, index) => {
    ...
    if (meta.route) {
      if (meta.route.startsWith("http://") || meta.route.startsWith("https://")) {
        window.open(meta.route, "_blank", "noopener,noreferrer");
      } else {
        navigate(meta.route);
      }
    }
