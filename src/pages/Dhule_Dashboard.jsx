...
const TITLE_TO_INDEX_MAP = {
    ...
    "legal": 16,
    "market": 17,
    "medicine inventory": 18,
    "open land": 19,
    "fire": 20,
    "works": 21,
    "marriage registration": 22,
...
export function getCardMeta(title, index) {
    if (title) {
        const mappedIndex = TITLE_TO_INDEX_MAP[title.toLowerCase()];
        if (mappedIndex !== undefined && CARD_META_LIST[mappedIndex]) {
            return CARD_META_LIST[mappedIndex];
        }
    }
...
/** Format current date: "20 Aug 2026 12:21 PM" (same as RTS Dashboard) */
...
export default function Dhule_Dashboard() {
    ...
    // Fetch dashboard cards for Dhule (ulbId = DHULE_ULB_ID)
    const fetchDashboard = useCallback(async () => {
        try {
            setLoading(true);
            setError(null);
            const res = await axios.get(`${API_BASE_URL}/dashboard/DashboardDataNew`, {
                params: { ulbId: DHULE_ULB_ID },
            });

            // Bind the date from API if it exists, otherwise fallback to current date/time
    ...
    const handleCardClick = (card, index) => {
        ...
        if (route) {
            if (route.startsWith("http://") || route.startsWith("https://")) {
                // For external links – append ulbId query param
                const separator = route.includes("?") ? "&" : "?";
                window.open(`${route}${separator}ulbId=${DHULE_ULB_ID}`, "_blank", "noopener,noreferrer");
            } else {
                // For internal routes – append ulbId as query param + pass via state
                const separator = route.includes("?") ? "&" : "?";
                const finalRoute = `${route}${separator}ulbId=${DHULE_ULB_ID}`;
                navigate(finalRoute, { state: { ulbId: DHULE_ULB_ID, from: "/dhuledashboard" } });
            }
        } else {
            // No route → open the Card Detail Page with status/charts/table
            const cardKey = (card.title || `card-${index}`)
                .toLowerCase()
                .replace(/\s+/g, "-")
                .replace(/[^a-z0-9-]/g, "");
            navigate(`/dhuledashboard/detail/${cardKey}`, {
                state: {
                    title: card.title,
                    color: meta.color,
